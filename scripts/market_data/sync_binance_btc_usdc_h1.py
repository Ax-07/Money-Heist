...
def fetch_ccxt_updates(
    *,
    ...
    try:
        ...
        overlap = next((item for item in fetched if int(item[0]) == last_existing.open_time), None)
        if overlap is None:
            raise SyncError(
                "Binance/CCXT n'a pas renvoyé la dernière bougie existante pour le contrôle "
                f"de recouvrement ({iso_utc(last_existing.open_time)})"
            )
        compare_overlap(last_existing, overlap)
