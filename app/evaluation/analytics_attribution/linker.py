...
from .resolver import AnalyticsSnapshotResolver
...
class OpportunityAnalyticsLinker:
    ...
    def __init__(
        ...
        *,
        ...
        self.resolver = AnalyticsSnapshotResolver(
            analytics_run=analytics_run,
            snapshots=snapshots,
        )
        # Keep the public attribute used by 24B.1 callers/tests.
        self.index = self.resolver.index

    def link(self, opportunity: OpportunityObservationRef) -> OpportunityAnalyticsLink:
        resolution = self.resolver.resolve(opportunity.observation)
