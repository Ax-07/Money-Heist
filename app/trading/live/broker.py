...
class KrakenSpotLiveBroker:
    (
        "Secure Spot broker boundary. Batch 14 remains disabled unless an external "
        "provider authorizes it."
    )
    ...
    async def submit_order(
        self, intent: LiveOrderIntent, *, constraints: MarketConstraints, reference_price: Decimal
    ) -> LiveOrderRecord:
        ...
        if remote is not None:
            ...
            self._event(
                "IDEMPOTENT_REMOTE_MATCH",
                "WARNING",
                intent,
                external_order_id=record.external_order_id or "",
            )
            return record

        pending = LiveOrderRecord(
            intent=intent, status=LiveOrderStatus.NEW, updated_at=self._utc_now()
        )
        ...
        if not isinstance(txids, list) or len(txids) != 1 or not str(txids[0]).strip():
            unknown = replace(
                pending,
                status=LiveOrderStatus.RECONCILIATION_REQUIRED,
                last_error_code="INVALID_ADD_ORDER_RESULT",
                updated_at=self._utc_now(),
            )
            self._store.put(unknown)
        ...
        record = replace(
            ...
            external_order_id=str(txids[0]),
            updated_at=self._utc_now(),
        )
        self._store.put(record)
        self._event(
            "ORDER_SUBMITTED", "INFO", intent, external_order_id=record.external_order_id or ""
        )
        return record
    ...
    async def cancel_order(self, client_order_id: str) -> LiveOrderRecord:
        if not self._cancellation_enabled:
            raise LiveExecutionDisabledError(
                "LIVE cancellation is not authorized by Batch 14 configuration"
            )
        local = self._store.get(client_order_id)
        if local is None:
        ...
        try:
            ...
            ambiguous = replace(
                local,
                status=LiveOrderStatus.RECONCILIATION_REQUIRED,
                last_error_code="AMBIGUOUS_CANCEL",
                updated_at=self._utc_now(),
            )
    ...
    async def reconcile(self, client_order_id: str) -> ReconciliationResult:
        ...
        if remote is None:
            if local is None:
                return ReconciliationResult(
                    client_order_id, ReconciliationFinding.ORDER_UNKNOWN, None
                )
            finding = (
                ReconciliationFinding.AMBIGUOUS
                if local.status
                in {
                    LiveOrderStatus.UNKNOWN,
                    LiveOrderStatus.RECONCILIATION_REQUIRED,
                ...
                else ReconciliationFinding.ORDER_UNKNOWN
            )
        ...
        if local is None:
            ...
            finding = (
                ReconciliationFinding.UNEXPECTED_FILL
                if remote_filled > 0
                else ReconciliationFinding.ORDER_UNKNOWN
            )
            return ReconciliationResult(
                ...
                finding,
                None,
                exchange_status=str(remote.get("status", "")),
                external_order_id=str(remote.get("txid", "")),
                exchange_filled_quantity=remote_filled,
                details={"reason": "exchange order exists without local record"},
            )
        ...
        if remote_volume != local.intent.quantity or record.filled_quantity > local.intent.quantity:
            ...
            local.external_order_id
            and record.external_order_id
            and local.external_order_id != record.external_order_id
        ):
        ...
        if finding is not ReconciliationFinding.CONSISTENT:
            self._event(
                "RECONCILIATION_DIVERGENCE", "CRITICAL", local.intent, finding=finding.value
            )
        return ReconciliationResult(
            client_order_id,
            finding,
    ...
    def _validate_intent(
        ...
        if intent.mode != "LIVE":
            raise LiveIntentValidationError("intent mode must be LIVE")
        if intent.system_id not in self._allowed_system_ids:
            raise LiveIntentValidationError("system_id is not allowlisted")
        if intent.symbol not in self._allowed_symbols:
            raise LiveIntentValidationError("symbol is not allowlisted")
        if intent.side not in {OrderSide.BUY, OrderSide.SELL}:
            raise LiveIntentValidationError("side is invalid")
        if intent.order_type not in {OrderType.MARKET, OrderType.LIMIT}:
            raise LiveIntentValidationError("order_type is invalid")
        if intent.is_stale(self._utc_now()):
            raise LiveIntentValidationError("order intent is stale")
        if reference_price <= 0 or not reference_price.is_finite():
            raise LiveIntentValidationError("reference_price must be finite and > 0")
        q = intent.quantity
        if q < constraints.min_qty:
            raise LiveIntentValidationError("quantity is below market minimum")
        if constraints.qty_step <= 0 or q % constraints.qty_step != 0:
            raise LiveIntentValidationError("quantity does not match market qty_step")
        if constraints.max_qty is not None and q > constraints.max_qty:
            raise LiveIntentValidationError("quantity exceeds market maximum")
        price = intent.limit_price if intent.limit_price is not None else reference_price
        if q * price < constraints.min_notional:
            raise LiveIntentValidationError("order notional is below market minimum")

    async def _find_exchange_order(self, client_order_id: str) -> Mapping[str, object] | None:
        for method, container in (
            (self._api.get_open_orders, "open"),
            (self._api.get_closed_orders, "closed"),
        ):
    ...
    def _record_from_exchange(
        self, intent: LiveOrderIntent, row: Mapping[str, object]
    ) -> LiveOrderRecord:
        ...
        if status in {"closed", "filled"}:
            mapped = LiveOrderStatus.FILLED
        elif status in {"canceled", "cancelled", "expired"}:
            mapped = LiveOrderStatus.CANCELED
        elif executed > 0 and executed < vol:
            mapped = LiveOrderStatus.PARTIALLY_FILLED
        elif status not in {"open", "pending"}:
            mapped = LiveOrderStatus.UNKNOWN
        avg_price = Decimal(str(row.get("price", "0")))
        return LiveOrderRecord(
            intent,
            mapped,
            str(row.get("txid", "")) or None,
            status,
            executed,
    ...
    def _event(
        self, event_type: str, severity: str, intent: LiveOrderIntent, **details: str
    ) -> None:
        self._audit.record(
            LiveAuditEvent(
                event_type,
    ...
    def _deadline(self) -> str:
        ...
        return (
            (self._utc_now() + timedelta(seconds=5))
    ...
    def _decimal_text(value: Decimal | None) -> str:
        if value is None:
            raise LiveIntentValidationError("missing decimal value")
