...
from app.trading.live.errors import (
...
CONSTRAINTS = MarketConstraints(
    qty_step=Decimal("0.001"), min_qty=Decimal("0.001"), min_notional=Decimal("1")
)


def intent(
    *, client="00000000-0000-0000-0000-000000000014", qty="0.01", created=None, expires=None
):
...
class FakeApi:
    ...
    async def get_closed_orders(self, *, client_order_id=None):
        return {"closed": self.closed, "count": len(self.closed)}
...
def broker(api, *, auth=True, cancellation=False):
    ...
    b = KrakenSpotLiveBroker(
        api=api,
        authorization=StaticLiveAuthorization(auth)
        if auth is not None
        else DenyAllLiveAuthorization(),
        store=store,
        audit=audit,
        allowed_system_ids=frozenset({"balanced_v1"}),
...
def test_batch14_default_authorization_is_fail_closed_even_with_broker_ready():
    api = FakeApi()
    b, _, audit = broker(api, auth=None)
    with pytest.raises(LiveExecutionDisabledError):
        asyncio.run(
            b.submit_order(intent(), constraints=CONSTRAINTS, reference_price=Decimal("50000"))
        )
...
def test_ambiguous_submission_is_blocked_until_reconciliation_and_never_resubmitted():
    api = FakeApi()
    api.add_error = LiveAmbiguousSubmissionError("timeout")
...
def test_reconciliation_resolves_ambiguous_order_when_exchange_finds_cl_ord_id():
    ...
    b, store, _ = broker(api)
    store.put(
        __import__("app.trading.live.models", fromlist=["LiveOrderRecord"]).LiveOrderRecord(
            intent(), LiveOrderStatus.RECONCILIATION_REQUIRED
        )
    )
...
def test_stale_intent_is_rejected_before_exchange_write():
    ...
    with pytest.raises(LiveIntentValidationError):
        asyncio.run(
            b.submit_order(stale, constraints=CONSTRAINTS, reference_price=Decimal("50000"))
        )
    assert api.add_calls == 0
...
def test_market_constraints_are_enforced_before_exchange_write():
    ...
    with pytest.raises(LiveIntentValidationError):
        asyncio.run(
            b.submit_order(
                intent(qty="0.0015"), constraints=CONSTRAINTS, reference_price=Decimal("50000")
            )
        )
    assert api.add_calls == 0
...
def test_withdrawal_permission_is_rejected_by_preflight():
    api = FakeApi()
    api.permissions = ["query-funds", "withdraw-funds"]
    b, _, _ = broker(api)
