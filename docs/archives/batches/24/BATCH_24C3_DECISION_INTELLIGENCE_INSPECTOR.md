...
Sections : Scanner ; Agents ; Decision Funnel ; Risk & Execution ; Analytics causal.
...
L'Inspector distingue explicitement `available_at`, `confirmed_at` et `operational_at` lorsqu'ils existent. Les fingerprints de cursor source et de snapshot Analytics sont affichés pour les opportunités ayant une attribution 24B.

## Dégradation contrôlée

- aucune sélection : invitation à sélectionner un objet ;
- objet Analytics sans opportunité : inspection sans requête Decision Intelligence ;
- ancien run sans sidecar 24C : indisponibilité explicite sans crash ;
- changement de rôle ou campagne : sélection effacée ;
- panneau fermé : sélection conservée.

## Invariants

- aucune mutation Scanner/Agents/Risk/PAPER ;
- aucun impact LIVE ;
